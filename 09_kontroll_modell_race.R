...
fit <- read_rds("fit/fit_vader_gridprop.rds") # Här tar vi enbart den bästa modellen
driver_samples      <- read_rds("fit/samples_koef_driver_race.rds")
constructor_samples <- read_rds("fit/samples_koef_team_race.rds")
...
mc_areas_driver    <- mcmc_areas(posterior_driver,
                                 ...
                                 prob = 0.95) + ggtitle("Posterior fördelningar Driver, Race",
                                                        "med median och 95% intervall") + 
  theme_fivethirtyeight() + theme(axis.title = element_text())
ggsave("img/posterior_samples_driver_race.png", width = 7, height = 10, bg = "white")
...
mcmc_plot(fit, type = "trace") +
  facet_wrap(~parameter, nrow = 6, scales = "free") + ggtitle("Chains Race") + theme_fivethirtyeight() + theme(axis.title = element_text())
ggsave("img/chains_race.png", width = 7, height = 10, bg = "white")
...
pred_tab_1955 <-
  f1_dat %>%
  filter(year == 1955) %>%
  select(driver, constructor, year, gridprop_trans) %>%
  distinct() %>%
...
ggsave("img/pp_check_prop_1955_race.png", width = 15, height = 12, bg = "white")
...
ggsave("img/pp_check_prop_1986_race.png", width = 15, height = 12, bg = "white")
...
ggsave("img/pp_check_prop_2007_race.png", width = 15, height = 12, bg = "white")


# 2021 posterior predictive check ----
# create drivers & constructors in 2021
pred_tab_2021 <-
  f1_dat %>%
  filter(year == 2021) %>%
  select(driver, constructor, year, gridprop_trans) %>%
...
# predict proportion of outperformed drivers
pp_tab_2021 <- posterior_predict(fit, pred_tab_2021)
...
ggsave("img/pp_check_prop_2021_race.png", width = 15, height = 12, bg = "white")
...
ggsave("img/pp_check_rank_2021_race.png", width = 15, height = 12, bg = "white")
...
ggsave("img/pp_check_rank_1986_race.png", width = 15, height = 12, bg = "white")
